...
export async function fetchOrderCourierInfo(orderId: string): Promise<OrderCourierInfo | null> {
  const { data, error } = await supabase
    ...
    .select('id, courier_company, tracking_number, courier_area, total_receivable, collected_amount, delivery_charge, cod_charge, delivery_discount, settlement_source, consignment_id, courier_status, courier_status_updated_at, courier_api_response, courier_api_error, total_receivable_modified_after_ship, total_receivable_ship_note')
    .eq('order_id', orderId)
    .maybeSingle();
  if (error) throw error;
